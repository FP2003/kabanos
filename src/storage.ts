...
export class SqlStorage implements StorageAdapter {
  ...
  async migrate(): Promise<void> {
    await this.db.schema.createTable('boards').ifNotExists().addColumn('id','varchar(64)',c=>c.primaryKey()).addColumn('name','varchar(255)',c=>c.notNull()).addColumn('created_at','varchar(32)',c=>c.notNull()).execute();
    await this.db.schema.createTable('columns').ifNotExists().addColumn('id','varchar(64)',c=>c.primaryKey()).addColumn('board_id','varchar(64)',c=>c.notNull()).addColumn('name','varchar(255)',c=>c.notNull()).addColumn('position','integer',c=>c.notNull()).addColumn('completion','integer',c=>c.notNull()).addColumn('color_token','varchar(64)').execute();
    await this.db.schema.createTable('cards').ifNotExists().addColumn('id','varchar(64)',c=>c.primaryKey()).addColumn('board_id','varchar(64)',c=>c.notNull()).addColumn('column_id','varchar(64)',c=>c.notNull()).addColumn('position','integer',c=>c.notNull()).addColumn('tag','varchar(64)',c=>c.notNull()).addColumn('title','text',c=>c.notNull()).addColumn('body','text',c=>c.notNull()).addColumn('notes','text',c=>c.notNull()).addColumn('assignee','varchar(255)').addColumn('labels','text',c=>c.notNull()).addColumn('priority','varchar(32)',c=>c.notNull()).addColumn('source_file_path','text',c=>c.notNull()).addColumn('source_line','integer',c=>c.notNull()).addColumn('source_content_hash','varchar(64)',c=>c.notNull()).addColumn('source_context_hash','varchar(64)',c=>c.notNull()).addColumn('source_occurrence','integer',c=>c.notNull()).addColumn('status','varchar(32)',c=>c.notNull()).addColumn('created_at','varchar(32)',c=>c.notNull()).addColumn('updated_at','varchar(32)',c=>c.notNull()).execute();
    await this.db.schema.createTable('resolved_fingerprints').ifNotExists().addColumn('id','varchar(64)',c=>c.primaryKey()).addColumn('board_id','varchar(64)',c=>c.notNull()).addColumn('card_id','varchar(64)',c=>c.notNull()).addColumn('file_path','text',c=>c.notNull()).addColumn('content_hash','varchar(64)',c=>c.notNull()).addColumn('context_hash','varchar(64)',c=>c.notNull()).addColumn('occurrence','integer',c=>c.notNull()).addColumn('resolved_at','varchar(32)',c=>c.notNull()).addColumn('resolved_by','varchar(255)').execute();
    await this.db.schema.createTable('settings').ifNotExists().addColumn('board_id','varchar(64)',c=>c.primaryKey()).addColumn('value','text',c=>c.notNull()).execute();
    await this.db.schema.createTable('activity_log').ifNotExists().addColumn('id','varchar(64)',c=>c.primaryKey()).addColumn('card_id','varchar(64)',c=>c.notNull()).addColumn('action','varchar(64)',c=>c.notNull()).addColumn('from_status','varchar(32)').addColumn('to_status','varchar(32)').addColumn('actor','varchar(255)').addColumn('created_at','varchar(32)',c=>c.notNull()).execute();
  }
  ...
  async reconcile(comments: SourceComment[]): Promise<{created:number;updated:number;archived:number}> {
    return this.db.transaction().execute(async trx => {
      ...
      const seen = new Set<string>(); let created=0, updated=0, archived=0;
      for (const comment of comments) {
        ...
        await trx.insertInto('cards').values({id,board_id:'default',column_id:column.id,position:cards.filter(c=>c.column_id===column.id).length+created,tag:comment.tag,title:comment.text,body:comment.rawText,notes:'',assignee:null,labels:JSON.stringify(tag?.label?[tag.label]:[]),priority:tag?.priority??'normal',source_file_path:comment.filePath,source_line:comment.line,source_content_hash:comment.contentHash,source_context_hash:comment.contextHash,source_occurrence:comment.occurrence,status:'open',created_at:now,updated_at:now}).execute();
        await log(trx,id,'created',null,'open',undefined); seen.add(id); created++;
      }
